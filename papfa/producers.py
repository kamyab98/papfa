...
class KafkaMessageProducer:
    def __init__(
        ...
        batch_config: BatchConfig = BatchConfig(size=100, timeout=timedelta(microseconds=10)),
        ...
        self.producer = SerializingProducer(
            {
                "bootstrap.servers": ",".join(kafka_producer_config.kafka_config.bootstrap_servers),
    ...
    def produce(self, messages: List[Record]):
        for message in messages:
            ...
            self.producer.produce(
                self.topic, value=message.value, key=message.key or _uuid, headers=message.headers or _uuid,
